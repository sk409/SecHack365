...
func main() {
	...
	http.Handle("/lessons/", cors(allowMethods([]string{http.MethodGet, http.MethodPost, http.MethodPut, http.MethodDelete}, preflight(&lessonsHandler{}))))
	http.Handle("/login", cors(allowCredentials(allowHeaders(headerContentType, preflight(&loginHandler{})))))
	http.Handle("/materials/", cors(allowMethods([]string{http.MethodGet, http.MethodPost, http.MethodPut, http.MethodDelete}, preflight(&materialsHandler{}))))
	...
	http.Handle("/users/", cors(allowHeaders([]string{goconst.HTTP_HEADER_CONTENT_TYPE}, allowMethods([]string{http.MethodGet, http.MethodPut}, preflight(&usersHandler{})))))
